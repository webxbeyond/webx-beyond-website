...
- High read QPS for redirects (reads >> writes)
- Low latency (aim <50ms at edge)
- High availability and graceful degradation
- Scalable analytics without slowing redirects
...
Basic APIs:
- POST /shorten { url, ttl?, custom_alias?, tags? } → { short_url }
- GET /:alias → 302 redirect to long URL
- GET /analytics/:alias → aggregated stats
...
SLO examples:
- Redirect success rate 99.99% monthly
- 95th percentile redirect latency < 30ms (edge), <100ms origin

Runbook items:
- High error rate → check cache tier & DB connectivity
- Event processing lag → check consumer lag, reprocess topic
- Abuse spike → rate limit or block offending IP ranges
